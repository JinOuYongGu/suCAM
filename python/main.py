...
class VView(QMainWindow):
    ...
    def update_ui(self):
        self.widget_arr['first_layer_thickness_edit'].setText(
            str(self.conf.get('first_layer_thickness')))
        self.widget_arr['layer_thickness_edit'].setText(
            str(self.conf.get('layer_thickness')))
        self.widget_arr['infill_offset_edit'].setText(
            str(self.conf.get('infill_offset')))
        self.update()
    ...
    def initUI(self):
        ...
        bt_height = 75
        bt_width = 200
        load_button = self.add_button(
            lw_layout, "Load Model", bt_width, bt_height)
        autolayout_button = self.add_button(
            lw_layout, "Auto Layout", bt_width, bt_height)
        slice_button = self.add_button(
            lw_layout, "Slice Model", bt_width, bt_height)
        fill_button = self.add_button(
            lw_layout, "Filling with Fermat Curve", bt_width, bt_height)
        connectp_button = self.add_button(
            lw_layout, "Connect Printer...", bt_width, bt_height)
        print_seq_button = self.add_button(
            lw_layout, "Print Sequence", bt_width, bt_height)
        print_path_button = self.add_button(
            lw_layout, "Generate Path", bt_width, bt_height)
        gcode_button = self.add_button(
            lw_layout, "Generate GCode", bt_width, bt_height)
        printto_button = self.add_button(
            lw_layout, "Print...", bt_width, bt_height)
        clear_button = self.add_button(lw_layout, "Clear", bt_width, bt_height)
        quit_button = self.add_button(
            lw_layout, "Exit Program", bt_width, bt_height)
        ...
        self.view_slice.sizeHint = self.view.sizeHint = lambda: pg.QtCore.QSize(
            640, 480)
    ...
    def openStlDialog(self):
        ...
        if fname[0]:
            ...
            self.message(self.mesh_info.get_info())
            verts = self.mesh.vectors.reshape(
                self.mesh.vectors.shape[0] * 3, 3)
    ...
    def gen_gcode(self):
        ...
        for line in gcode:
            f.writelines(line + "\n")
        f.close
        #np.savetxt(self.gcode_path + '.gcode', self.gcode, fmt='%.6f')
        return

...
class TriZSlice:
    def __init__(self, optionverbose):
        ...
        # though these may have a better definition than
        # triangle files
        # such as branching lines and variable offset radii

    def LoadSTLfile(self, stlfile, transmap):
        tbm = TriangleBarMesh(stlfile, transmap, nodesortkey=lambda X: (
            X[0][2], X[0][1], X[0][0], X[1]))  # every edge has nodefrom.p.z<=nodeto.p.z
        if self.optionverbose:
            nnodes, nedges, ntriangles, nsinglesidededges = tbm.GetFacts()
            print("%s:  nodes=%d edges=%d triangles=%d singlesidededges=%d" %
                  (stlfile, nnodes, nedges, ntriangles, nsinglesidededges))
            if nsinglesidededges != 0:
                print("*** Warning, not a closed surface")
        self.tbms.append(tbm)

    def SetExtents(self, optionextra):
        self.xlo, self.xhi = min(tbm.xlo for tbm in self.tbms), max(
            tbm.xhi for tbm in self.tbms)
        self.ylo, self.yhi = min(tbm.ylo for tbm in self.tbms), max(
            tbm.yhi for tbm in self.tbms)
        self.zlo, self.zhi = min(tbm.zlo for tbm in self.tbms), max(
            tbm.zhi for tbm in self.tbms)
        if self.optionverbose:
            print("Dimensions: xlo=%.3f xhi=%.3f  ylo=%.3f yhi=%.3f  zlo=%.3f zhi=%.3f" % (
    ...
    def BuildPixelGridStructures(self, optionwidthpixels, optionheightpixels):
        ...
        if optionheightpixels == 0:
            heightpixels = int(
                optionwidthpixels / (self.xhi - self.xlo) * (self.yhi - self.ylo) + 1)
            newyhi = heightpixels * \
                (self.xhi - self.xlo) / optionwidthpixels + self.ylo
            if self.optionverbose:
                print("Revising yhi from %.3f to %.3f to for a whole number of pixels" % (
                    self.yhi, newyhi))
            self.yhi = newyhi
        else:
        ...
        if self.optionverbose:
            print("numxpixels=%d  each width=%.3f  x0=%0.3f" %
                  (self.xpixels.nparts, xpixwid, self.xpixels.vs[0]))
            print("numypixels=%d  each height=%.3f  y0=%0.3f" %
                  (self.ypixels.nparts, ypixwid, self.ypixels.vs[0]))

        # partitions with interval boundaries down middle of each pixel with
        # extra line each side for convenience
        self.xpixmidsE = Partition1(
            self.xlo - xpixwid * 0.5, self.xhi + xpixwid * 0.5, self.xpixels.nparts + 1)
        self.ypixmidsE = Partition1(
            self.ylo - ypixwid * 0.5, self.yhi + ypixwid * 0.5, self.ypixels.nparts + 1)
    ...
    def CalcPixelYcuts(self, z, tbm):
        ...
        for bar in tbm.bars:   # bucketing could speed this up
            ...
            if bar.nodeback.p.z <= z < bar.nodefore.p.z:
                ...
                lam = (z - bar.nodeback.p.z) / \
                    (bar.nodefore.p.z - bar.nodeback.p.z)
        ...
        for i, i1 in tbarpairs:
            ...
            iyl, iyh = self.ypixmidsE.GetPartRange(
                min(p0.v, p1.v), max(p0.v, p1.v))
    ...
    def CalcYsegrasters(self, z):
        ...
        # work through each raster line across the list of stlfiles
        for iy in range(self.ypixels.nparts):
    ...
    def WritePNG(self, fout, lcompressed):
        ...
        block = struct.pack("!I4sIIBBBBB", 13, "IHDR".encode(
            'ascii'), widthpixels, heightpixels, bitdepth, colortype, compression, filtertype, interlaced)
        fout.write(block)
        fout.write(struct.pack("!I", zlib.crc32(block[4:]) & 0xFFFFFFFF))
        # length of compressed data at start of compressed section (which is why it
        fout.write(struct.pack("!I", sum(map(len, lcompressed))))
        # can't be streamed out)
    ...
    def SliceToPNG(self, z, pngname):
        ...
        if self.optionverbose:
            print("Sliced at z=%f to file %s  compressbytes=%d %dms" % (
                z, pngname, sum(map(len, lcompressed)), (time.time() - stime) * 1000))
